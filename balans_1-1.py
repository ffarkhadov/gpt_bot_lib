...
A  Дата обновления │ B  SKU │ C  Наименование │ D  Поставка │ E  Свободный остаток
F  Утиль/Возврат   │ G  Едет на склад Ozon │ H… статусы («В пути», «delivered» и др.)

Правила
───────
• Строки уникальны по SKU.  При повторном запуске строка ПОЛНОСТЬЮ перезаписывается.
• Статусы заказов `awaiting_deliver / awaiting_packaging / delivering`
  суммируются в колонку **«В пути»**.
• `cancelled` полностью игнорируется.
• Из метода `/v1/returns/list` собираем возвраты и распределяем их по двум колонкам:
  **«Утиль/Возврат»** и **«Едет на склад Ozon»** согласно `status_map`.
"""
