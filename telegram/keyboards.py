...
def kb_store_menu(sid: str) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(
            text="📝 Обновить unit-day", callback_data=f"unit_{sid}")],
        [InlineKeyboardButton(
            text="📊 Обновить баланс",  callback_data=f"balans_{sid}")],
        [InlineKeyboardButton(
            text="🔄 Запустить авто-обновление", callback_data=f"update_{sid}")],
        [InlineKeyboardButton(
            text="⏹ Остановить обновление", callback_data=f"stop_{sid}")],
    ])
